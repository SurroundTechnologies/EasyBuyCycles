...
namespace WPF.ShippingAddress
{
	...
	public partial class  ShippingAddressDropDown : AB_DropDownBase
	{
		...
		{
			get => (int?)GetValue(ShippingAddressInternalIDProperty);
			set => SetValue(ShippingAddressInternalIDProperty, value);
		}
		...
		protected override void am_SetParentProperties()
		{
			...
			ap_CurrentView = "YD1SLF1";
			
			//The name of the property that should be displayed in the Drop Down
			ap_ComboBoxDisplayMemberPath = ShippingAddressEntity.NameProperty.ap_PropertyName;
			...
			//ap_KeyDependencyProperties = new DependencyProperty[] { ShippingAddressInternalIDProperty};     
		...
		/// </summary>
		public void SetValue(int? ShippingAddressInternalID)    
		{
			...
			am_OnIdChanged(new DependencyPropertyChangedEventArgs(ShippingAddressInternalIDProperty, null, ShippingAddressInternalID));
		}
	}
}