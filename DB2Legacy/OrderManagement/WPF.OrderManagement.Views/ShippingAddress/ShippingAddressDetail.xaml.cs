...
namespace WPF.ShippingAddress
{
	...
	{        
		...
		{
            var selectedEntity = ap_CurrentEntity as ShippingAddressEntity;

            switch (command.ap_CommandID)
            {
                case Constants.CMD_CopyAddressLine:
                    Utilities.CopyToClipboard(selectedEntity.ShippingAddressLine, ap_MessageConsole);
                    e.Handled = true;
                    break;
                case Constants.CMD_CopyAddressBlock:
                    Utilities.CopyToClipboard(selectedEntity.ShippingAddressBlock, ap_MessageConsole);
                    e.Handled = true;
                    break;
                case Constants.CMD_OpenInMaps:
                    Utilities.OpenWithGoogleMaps(selectedEntity.ShippingAddressLine, ap_MessageConsole);
                    e.Handled = true;
                    break;
