...
namespace WPF.OrderItem
{
	...
	public partial class  OrderItemDropDown : AB_DropDownBase
	{
		...
		public int? OrderItemInternalID
		{
			get => (int?)GetValue(OrderItemInternalIDProperty);
			set => SetValue(OrderItemInternalIDProperty, value);
		}
        public static readonly DependencyProperty OrderItemInternalIDProperty = DependencyProperty.Register(nameof(OrderItemInternalID), typeof(int?), typeof(OrderItemDropDown), new FrameworkPropertyMetadata(null, FrameworkPropertyMetadataOptions.BindsTwoWayByDefault, new PropertyChangedCallback(AB_DropDownBase.IdChanged)));
		...
		protected override void am_SetParentProperties()
		{
			...
			ap_ComboBoxDisplayMemberPath = OrderItemEntity.OrderItemInternalIDProperty.ap_PropertyName;
			...
			//ap_KeyDependencyProperties = new DependencyProperty[] { OrderItemInternalIDProperty};     
		...
		/// </summary>
		public void SetValue(int? OrderItemInternalID)    
		{
			// Turn off property change event triggers
			ap_KeysPopulating = true;

			this.OrderItemInternalID = OrderItemInternalID;   
		
			// Turn on property change event triggers, and fire an event to respond to the changed keys.
			ap_KeysPopulating = false;
			am_OnIdChanged(new DependencyPropertyChangedEventArgs(OrderItemInternalIDProperty, null, OrderItemInternalID));
		}
	}
}