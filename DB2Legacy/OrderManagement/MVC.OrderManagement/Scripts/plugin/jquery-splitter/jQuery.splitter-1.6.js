 ...
 *
 *
 * @example $jq('#MySplitter').splitter();
 * @desc Create a vertical splitter with default settings 
 *
 * @example $jq('#MySplitter').splitter({type: 'h', accessKey: 'M'});
...
var splitterCounter = 0;
 ...
 $jq.fn.splitter = function(args){
	args = args || {};
	return this.each(function() {
		if ( $jq(this).is(".splitter") )	// already a splitter
			return;
		...
		var splitter = $jq(this).css({position: "relative"}).addClass(opts.splitterClass);
		var panes = $jq(">*", splitter[0]).addClass(opts.paneClass).css({
			position: "absolute", 			// positioned inside splitter container
			"z-index": "1",					// splitbar is positioned above
			"-moz-outline-style": "none"	// don't show dotted outline
		});
		var A = $jq(panes[0]), B = $jq(panes[1]);	// A = left/top, B = right/bottom
		opts.dockPane = opts.dock && (/right|bottom/.test(opts.dock)? B:A);

		// Focuser element, provides keyboard support; title is shown by Opera accessKeys
		var focuser = $jq('<a href="javascript:void(0)"></a>')
			.attr({accessKey: opts.accessKey, tabIndex: opts.tabIndex, title: opts.splitbarClass})
			.bind(($jq.browser.opera?"click":"focus")+opts.eventNamespace, 
		...
		var bar = $jq('<div></div>')
			...
			.bind("mouseover"+opts.eventNamespace, function(){
				$jq(this).addClass(opts.barHoverClass);
			})
			.bind("mouseout"+opts.eventNamespace, function(){
				$jq(this).removeClass(opts.barHoverClass);
			});
		// Use our cursor unless the style specifies a non-default cursor
		if ( /^(auto|default|)$jq/.test(bar.css("cursor")) )
		...
		A._pane = opts.side1;
		B._pane = opts.side2;
		$jq.each([A,B], function(){
			this._splitter_style = this.style;
			this._min = opts["min"+this._pane] || dimSum(this, "min-"+opts.split);
			this._max = opts["max"+this._pane] || dimSum(this, "max-"+opts.split) || 9999;
			this._init = opts["size"+this._pane]===true ?
				parseInt($jq.curCSS(this[0],opts.split),10) : opts["size"+this._pane];
		});
		...
		if ( opts.cookie ) {
			...
			initPos = parseInt($jq.cookie(opts.cookie),10);
			$jq(window).bind("unload"+opts.eventNamespace, function(){
				var state = String(bar.css(opts.origin));	// current location of splitbar
				$jq.cookie(opts.cookie, state, {expires: opts.cookieExpires || 365, 
		...
		if ( opts.resizeTo ) {
			...
			$jq(window).bind("resize"+opts.eventNamespace, function(){
				var top = splitter.offset().top;
				var eh = $jq(opts.resizeTo).height();
				splitter.css("height", Math.max(eh-top-splitter._hadjust, splitter._hmin)+"px");
				if ( !$jq.browser.msie ) splitter.trigger("resize");
			}).trigger("resize"+opts.eventNamespace);
		}
		else if ( opts.resizeToWidth && !$jq.browser.msie ) {
			$jq(window).bind("resize"+opts.eventNamespace, function(){
		...
		if ( opts.dock ) {
			...
			if ( opts.dockKey )
				$jq('<a title="'+opts.splitbarClass+' toggle dock" href="javascript:void(0)"></a>')
					.attr({accessKey: opts.dockKey, tabIndex: -1}).appendTo(bar)
					.bind($jq.browser.opera?"click":"focus", function(){ 
		...
		splitter
			.bind("destroy"+opts.eventNamespace, function(){
				$jq([window, document]).unbind(opts.eventNamespace);
				bar.unbind().remove();
