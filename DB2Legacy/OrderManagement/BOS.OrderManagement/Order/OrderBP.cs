    ...
    public class OrderBP : AB_FrameworkBusinessObjectProcessBase<OrderEntity>, IOrderBPServiceContract
    {
        ...
        public override AB_ProcessRequestReturnArgs am_ProcessRequest(AB_ProcessRequestInputArgs inputArgs)
        {
            switch (inputArgs.ap_CommandID)
            {
                ...
                case "ASYNCPROCESSORDERWIZARD":
                    var processArgs = AsyncProcessOrderWizard(inputArgs);
                    return new AB_ProcessRequestReturnArgs(processArgs.ap_ReturnCode, processArgs.ap_Messages);
        ...
        private AB_InsertReturnArgs AsyncProcessOrderWizard(AB_ProcessRequestInputArgs inputArgs)
        {
            return am_CallWithTransaction(inputArgs, (inArgs) =>
            {
                var wizardObject = inputArgs.am_GetCustomInputArgs<OrderWizardObject>();

                wizardObject.ProgressMessage = DescriptionResource.PROCESSINGSTARTED;
                wizardObject.ProgressPercentage = 0;

                //Customer insertion if needed
                if (!wizardObject.IsSpecifyingExistingCustomer)
                {
                    wizardObject.ProgressMessage = DescriptionResource.CREATINGNEWCUSTOMER;

                    using (var customerBP = new CustomerBP())
                    {
                        var custInsertInArgs = new AB_InsertInputArgs<CustomerEntity>(wizardObject.CurrentCustomerEntity).am_InitializeWith(inArgs);
                        var custInsertRetArgs = customerBP.am_Insert(custInsertInArgs);
                        if (custInsertRetArgs.ap_ReturnCode != AB_ReturnCodes.OK.ToString())
                        {
                            wizardObject.ProgressMessage = DescriptionResource.ERRORCREATINGNEWCUSTOMER;
                            wizardObject.ProgressPercentage = 100;

                            return custInsertRetArgs;
                        }
                        wizardObject.CurrentCustomerEntity = (CustomerEntity)custInsertRetArgs.ap_OutputEntity;
                        wizardObject.ProgressPercentage = 25;
                    }
                }

                //shipping address insertion
                if (!wizardObject.IsSpecifyingExistingShippingAddress)
                {
                    wizardObject.ProgressMessage = DescriptionResource.CREATINGNEWSHIPPINGADDRESS;
                    wizardObject.CurrentShippingAddressEntity.CustomerInternalID = wizardObject.CurrentCustomerEntity.CustomerInternalID;

                    using (var shippingAddressBP = new ShippingAddressBP())
                    {
                        var shipAddrInsertInArgs = new AB_InsertInputArgs<ShippingAddressEntity>(wizardObject.CurrentShippingAddressEntity).am_InitializeWith(inArgs);
                        var shipAddrInsertRetArgs = shippingAddressBP.am_Insert(shipAddrInsertInArgs);

                        if (shipAddrInsertRetArgs.ap_ReturnCode != AB_ReturnCodes.OK.ToString())
                        {
                            wizardObject.ProgressMessage = DescriptionResource.ERRORCREATINGNEWSHIPPINGADDRESS;
                            wizardObject.ProgressPercentage = 100;

                            return shipAddrInsertRetArgs;
                        }

                        wizardObject.CurrentShippingAddressEntity = (ShippingAddressEntity)shipAddrInsertRetArgs.ap_OutputEntity;
                        wizardObject.ProgressPercentage = 50;
                    }
                }

                //Order insertion
                wizardObject.ProgressMessage = DescriptionResource.CREATINGNEWORDER;

                wizardObject.CurrentOrderEntity.OrderTime = DateTime.Now.TimeOfDay;
                wizardObject.CurrentOrderEntity.OrderDate = DateTime.Today;
                wizardObject.CurrentOrderEntity.ShippingAddressInternalID = wizardObject.CurrentShippingAddressEntity.ShippingAddressInternalID;
                wizardObject.CurrentOrderEntity.CustomerInternalID = wizardObject.CurrentCustomerEntity.CustomerInternalID;

                var orderInsertInArgs = new AB_InsertInputArgs<OrderEntity>(wizardObject.CurrentOrderEntity).am_InitializeWith(inArgs);
                var orderInsertRetArgs = am_Insert(orderInsertInArgs);

                if (orderInsertRetArgs.ap_ReturnCode != AB_ReturnCodes.OK.ToString())
                {
                    wizardObject.ProgressMessage = DescriptionResource.ERRORCREATINGNEWORDER;
                    wizardObject.ProgressPercentage = 100;

                    return orderInsertRetArgs;
                }

                wizardObject.CurrentOrderEntity = (OrderEntity)orderInsertRetArgs.ap_OutputEntity;
                wizardObject.ProgressPercentage = 75;

                //Order Items insertion
                wizardObject.ProgressMessage = DescriptionResource.CREATINGNEWORDERITEMS;
                using (var orderItemBP = new OrderItemBP())
                {

                    foreach (OrderItemEntity orderItem in wizardObject.OrderItems)
                    {
                        orderItem.OrderInternalID = wizardObject.CurrentOrderEntity.OrderInternalID;
                        var orderItemInsertInArgs = new AB_InsertInputArgs<OrderItemEntity>(orderItem).am_InitializeWith(inArgs);
                        var orderItemInsertRetArgs = orderItemBP.am_Insert(orderItemInsertInArgs);

                        if (orderItemInsertRetArgs.ap_ReturnCode != AB_ReturnCodes.OK.ToString())
                        {
                            wizardObject.ProgressMessage = DescriptionResource.ERRORCREATINGNEWORDERITEMS;
                            wizardObject.ProgressPercentage = 100;
                    ...
                    wizardObject.ProgressPercentage = 100;
                    wizardObject.ProgressMessage = DescriptionResource.PROCESSINGCOMPLETE;
                }
